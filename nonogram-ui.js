...
(function(root,factory){
  ...
  const api=factory(logic,sessions,root);
  if(typeof module==='object'&&module.exports)module.exports=api;
  if(root)root.QuadludNonogramUI=api;
})(typeof globalThis!=='undefined'?globalThis:this,function(Logic,SessionAdapters,root){
'use strict';
...
function createAdapter(deps){
  ...
  const {document,query,shell,getCurrent,isPaused,a11ySetupGrid,a11yAnnounce,a11ySetCell,applyLogicalMove}=deps;
  const hint=typeof deps.hint==='function'?deps.hint:null;
  const GridCoordinates=deps.gridCoordinates||root?.QuadludGridCoordinates;
  if(!GridCoordinates||typeof GridCoordinates.rowLabel!=='function'||typeof GridCoordinates.columnLabel!=='function'||typeof GridCoordinates.coordinateLabel!=='function')throw new Error('QUADLUD Nonogram UI dependency unavailable: gridCoordinates');
  ...
  function applyAt(r,c){
    ...
    const move=SessionAdapters.nonogram.createCellMove(r,c,next);applyLogicalMove(move);draw();haptic(4);a11yAnnounce(`${GridCoordinates.coordinateLabel(r,c)}, ${stateLabel(next,labels)}`);maybeAutoFinish();return true
  }
  ...
  function syncAccessibility(){
    ...
    [...board.children].forEach((cell,i)=>{const r=Math.floor(i/s.puzzle.cols),c=i%s.puzzle.cols,state=s.state[r][c],coordinate=GridCoordinates.coordinateLabel(r,c);cell.dataset.coordinate=coordinate;a11ySetCell(cell,r,c,`${coordinate}, ${stateLabel(state,labels)}`);cell.setAttribute('aria-describedby',`${clueDomId('row',r)} ${clueDomId('col',c)}`)});return true
  }
  function draw(){
  ...
  function buildCells(session){
    ...
    for(let r=0;r<rows;r++)for(let c=0;c<cols;c++){const cell=document.createElement('div');cell.className='cell ng-cell ng-unknown';cell.dataset.r=String(r);cell.dataset.c=String(c);cell.dataset.coordinate=GridCoordinates.coordinateLabel(r,c);cell.dataset.entityKind='cell';cell.dataset.entityId=Logic.cellId(r,c);if((c+1)%5===0&&c<cols-1)cell.classList.add('ng-group-right');if((r+1)%5===0&&r<rows-1)cell.classList.add('ng-group-bottom');cell.addEventListener('click',()=>applyAt(r,c));board.appendChild(cell)}
